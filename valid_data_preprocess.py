...
parser.add_argument("--data_path", type=str, default='/home/data/SSROriDataset/CVAE/', help="data path")
...
parser.add_argument("--valid_data_path", type=str, default='data/2020CLEAN/2020Valid', help="preprocess_data_path")
...
def process_data(patch_size, stride, mode):
    if mode == 'valid':
        print("\nprocess valid set ...\n")
        patch_num = 1
        filenames_hyper = glob.glob(os.path.join(opt.data_path, 'Valid_mat', '*.mat'))
        filenames_rgb = glob.glob(os.path.join(opt.data_path, 'Valid_rgb', '*.png'))
